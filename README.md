...
this Mac ignores heights just as silently, owner bit or not — see
*Pairing* below.
...
**A desk will not move unless it is paired with this Mac, and nothing
says so.** Unpaired, it takes reads, ownership and Control commands and
silently drops every height. macOS offers to pair in a **Connection
Request** dialog shortly after connecting: click *Connect*.

[PAIRING.md](PAIRING.md) covers what *Cancel* and an unanswered dialog
leave behind, why a desk connected to another Mac shows up in a scan but
cannot be connected to, and the disconnect loops after re-pairing that
pairing mode on the desk put right.
...
`Move` returns as soon as the move is under way; a background goroutine
keeps the target in front of the desk until it arrives. In short:

- **A stop short of the target ends the move** and is never
  re-commanded, since it may be an obstruction. A collision the desk
  reports ends it at once.
- **A new target further on in the same direction is queued** until the
  desk reaches the current one. Any other new target brings the desk to
  rest first — at full speed it coasts on about 40 mm — because a
  different height reaching a moving desk halts it.
- **Every new height waits for a second of silence** (`RetargetGap`):
  the controller ignores one that comes sooner.
- **A target within 1.2 mm of the desk is a no-op** — the controller's
  dead band.
- **A move that never starts is given up on after 2.5 s.** The usual
  reasons are the dead band, ownership, and pairing.

Why, with the measurements behind each rule: [MOVING.md](MOVING.md).
...
- [MOVING.md](MOVING.md) — how `Move` drives the desk, and the
  measurements behind it
- [PAIRING.md](PAIRING.md) — pairing, the Connection Request dialog, and
  desks that can be seen but not connected to
